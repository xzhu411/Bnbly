...
const MyReservationsPage = () => {
    ...
    const fetchAll = () => {
        ...
        Promise.all([
            authFetch(`${API_BASE}/api/reservations/my/`, {
                headers: { Authorization: `Bearer ${accessToken}` },
            }).then(r => r.json()),
            authFetch(`${API_BASE}/api/reservations/hosting/`, {
                headers: { Authorization: `Bearer ${accessToken}` },
            }).then(r => r.json()),
        ]).then(([guest, host]) => {
    ...
    const handleCancelAsGuest = async (id: string) => {
        ...
        setCancellingId(id);
        const res = await authFetch(`${API_BASE}/api/reservations/cancel/${id}/`, {
            method: 'DELETE',
            headers: { Authorization: `Bearer ${accessToken}` },
        });
    ...
    const handleCancelAsHost = async (id: string) => {
        ...
        setCancellingId(id);
        const res = await authFetch(`${API_BASE}/api/reservations/host-cancel/${id}/`, {
            method: 'DELETE',
            headers: { Authorization: `Bearer ${accessToken}` },
        });
    ...
    const getImageSrc = (image: string | null) =>
        image ? (image.startsWith('http') ? image : `${API_BASE}${image}`) : '/beach1.jpeg';
        ...
        <main className="max-w-[1500px] mx-auto px-6 pb-6">
            ...
            {!isAuthenticated() && (
                <div className="text-center py-20 text-gray-400">
                    <p className="text-lg">Please log in to view your reservations</p>
                </div>
            )}
